from app.providers.azure.instances import get_instances
from app.providers.azure.metrics import get_cpu_metrics, get_network_metrics
...
def scan_azure(observation_days: int = 7) -> dict:
    all_instances = []
    idle_resources = []
    total_waste_kwh = 0.0

    for inst in get_instances():
        cpu = get_cpu_metrics(inst["instance_id"], observation_days)
        net = get_network_metrics(inst["instance_id"], observation_days)
